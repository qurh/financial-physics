# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 11:33:56 2024
...
def build_network(L, p):
    """
    Builds a small-world network by rewiring links in a square lattice.
    ...
    Returns:
        network (dict): Adjacency list representing the network.
    """
...
def majority_vote_dynamics(network, L, q, p, num_steps):
    """
    Simulates the three-state majority-vote dynamics on a given network.

    Args:
        network (dict): Adjacency list representing the network.
        L (int): Linear size of the network.
        q (float): Noise parameter.
        p (float): Rewiring probability used to construct the network.
        num_steps (int): Number of Monte Carlo steps.

    Returns:
        states (numpy.ndarray): Final states of the nodes after the simulation.
    """
    N = L ** 2  # Total number of nodes
...
def calculate_magnetization(states, N):
    ...
    Args:
        states (numpy.ndarray): Current states of the nodes.
        N (int): Total number of nodes.

    Returns:
        float: Average magnetization of the system.
    """
...
def calculate_susceptibility(states, N, M):
    ...
    Args:
        states (numpy.ndarray): Current states of the nodes.
        N (int): Total number of nodes.
        M (float): Average magnetization of the system.

    Returns:
        float: Magnetic susceptibility of the system.
    """
    state_counts = [np.count_nonzero(states == state) for state in [1, 2, 3]]
    m1 = (3 / 2) * (state_counts[0] / N - 1 / 3)
    m2 = (3 / 2) * (state_counts[1] / N - 1 / 3)
...
def calculate_binder_cumulant(states, N, M):
    """
    Calculates the Binder cumulant of the system.
    ...
    Returns:
        float: Binder cumulant of the system.
    """
    state_counts = [np.count_nonzero(states == state) for state in [1, 2, 3]]
    m1 = (3 / 2) * (state_counts[0] / N - 1 / 3)
    m2 = (3 / 2) * (state_counts[1] / N - 1 / 3)
...
def finite_size_scaling(L_values, q_values, p_values, num_steps):
    """
    Performs finite-size scaling analysis for the three-state majority-vote model.

    Args:
        L_values (list): List of linear system sizes.
        q_values (numpy.ndarray): Range of noise parameter values.
        p_values (list): List of rewiring probabilities.
        num_steps (int): Number of Monte Carlo steps.

    Returns:
        dict: Dictionary containing the critical noise parameters, critical exponents,
