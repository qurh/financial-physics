...
def worker(args):
    ...
    M = calculate_magnetization(states, L**2)
    chi = calculate_susceptibility(states, L**2, M)
    U = calculate_binder_cumulant(states, L**2, M)
    return M, chi, U
...
if __name__ == "__main__":
    ...
    for p in p_values:
        ...
        for L in L_values:
            ...
            results = pool.map(worker, [(L, q, p) for q in q_values])
            pool.close()
            
            M_L, chi_L, U_L = zip(*results)
