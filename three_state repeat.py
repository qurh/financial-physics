...
num_steps = 10 ** 6  # Number of Monte Carlo steps
...
for p in p_values:
    ...
    ln_q_c_diff = np.log(np.array(q_c_estimates) - q_c)
    ...
    # Estimate gamma/nu
    ln_chi = np.log(np.array([chi_data[i][np.searchsorted(q_values, q_c)] for i in range(len(L_values))]))
